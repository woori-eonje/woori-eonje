...
- **추천 우선순위(8단계 안정 정렬)**: ①필수참석자 전원 가능 → ②연속 구간 내 최소 가능 인원 多 → ③평균 점수 高 → ④불가 인원 少 → ⑤애매 인원 少 → ⑥미응답자 少 → ⑦빠른 날짜 → ⑧생성 순서(id/start_at 오름차순). 8단계까지 동일하면 항상 같은 결과를 반환(결정론적).
  - **구간 상태 집계**: 한 참여자의 구간 상태는 N개 슬롯 중 **하나라도 미응답이면 '미응답'**, 아니면 **min(가장 나쁜 상태)** — 전부 가능=가능, 하나라도 불가=불가, 그 외(가능+애매 등)=애매.
  - **③의 `score`**: 실제 저장·반환 값은 "평균"이 아니라 **구간 내 (참여자×슬롯) 점수 '합'**이다. 한 모임의 모든 후보는 동일 N·동일 참여자 집합이라 합 정렬 = 평균 정렬(단조 동치)이므로 ③ 순위는 같고, 정수로 다루기도 자연스러워 합으로 저장한다.

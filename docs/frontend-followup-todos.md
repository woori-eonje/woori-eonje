...
- [x] `develop-fetch`의 최신 백엔드 변경을 `develop-front`에 반영
- [x] `packages/types/src/index.ts`에 신규 요청·응답 타입 확인
- [x] `openapi/openapi.yaml`의 실제 경로와 오류 코드 확인
- [x] API 서버 `3001`, 프론트 서버 `3000` 실행
...
- [x] 현재 `summary.total === 0` 제출 버튼 비활성화 유지
- [x] 선택하지 않은 상태에서 제출 버튼 비활성화
- [x] API가 `AVAILABILITY_REQUIRED`를 반환하면 사용자 메시지 표시
- [x] 기존 응답 수정 중 전체 선택을 지우면 제출 버튼 비활성화
...
- [x] `getVoteDetails(meetingId)` 추가
- [x] 인증이 필요한 모임장 전용 요청으로 연결
- [x] 로딩·403·404·일반 오류 처리 및 재시도 UI 보완
...
- [x] 히트맵 슬롯을 클릭 가능하게 변경
- [x] 선택한 슬롯의 가능·애매·불가 참여자 명단 표시
- [x] 응답이 없는 상태의 빈 화면 처리
- [x] 모바일에서는 상세 카드가 히트맵 아래로 쌓이는 방식 사용
...
- [x] 추천 카드에 참여자 상세 표시
- [x] 가능·애매·불가·미응답으로 참여자를 분류
- [x] 필수 참여자 표시
- [ ] 기존 인원수와 상세 명단 수가 일치하는지 확인

### 4단계: 확정 일정 상세

- [x] 확정된 시작·종료 시각에 해당하는 추천 상세 찾기
- [x] 확정 일정 기준 가능·애매·불가·미응답 명단 표시
- [x] 현재 전체 참여자 칩을 상태별 명단으로 확장
- [ ] 전체 참여자 수와 상태별 합계 확인
...
- [x] API 클라이언트에 `deleteParticipant()` 추가
- [x] 참여자 관리 목록에 삭제 버튼 추가
- [x] 삭제 확인 모달 추가
- [x] 삭제 중 버튼 중복 실행 방지
- [x] 성공 후 참여자 목록·집계·추천 결과 다시 조회
- [x] 확정 화면에서는 삭제 UI 미노출
- [x] 실패 오류 메시지 표시
...
- [x] 신규 참여와 기존 응답 불러오기 전환 추가
- [x] 로그인 회원 참여 흐름은 기존 방식 유지
- [x] 비회원에게만 참여 PIN 입력 노출
...
- [x] 숫자 4자리 PIN 입력
- [x] PIN 확인 입력
- [x] 숫자 키패드가 뜨도록 `inputMode="numeric"` 사용
- [x] 4자리·확인 일치 검증
- [x] 닉네임 중복 오류 표시
- [x] 성공 시 기존 `participantEditToken` 저장 후 시간 선택 화면 이동

### 3단계: 기존 응답 재접속

- [x] 닉네임+PIN 입력 폼 추가
- [x] 세션 API 호출
- [x] 성공 시 받은 participant 정보를 기존 저장 구조에 기록
- [x] 시간 선택 화면에서 기존 응답 조회
- [x] 인증 실패와 잠금 오류를 구분해 안내
...
- [x] 비밀번호 재확인 없이 탈퇴할 수 없음
- [x] 탈퇴 성공 후 보호 페이지에 접근할 수 없음
- [x] 로컬 인증 정보가 남지 않음
- [x] 뒤로 가기·직접 URL로 인증 화면이 다시 노출되지 않음
...
- [x] 로그인 폼 아래 `비밀번호를 잊으셨나요?` 링크 추가

### 2단계: 이메일 요청 화면

- [x] `/forgot-password` 페이지 추가
- [x] 이메일 입력 및 형식 검증
- [x] 성공 시 계정 존재 여부와 무관하게 같은 완료 화면 표시
...
- [x] `/reset-password?token=...` 페이지 추가
- [x] URL token 누락 처리
- [x] 새 비밀번호와 확인 입력
- [x] 8~72자 및 일치 검증
- [x] 성공 후 로그인 화면으로 이동
- [x] 만료·사용된 링크 오류 화면 제공

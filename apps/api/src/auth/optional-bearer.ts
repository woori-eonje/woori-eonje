...
// - 서명은 유효해도 비밀번호 재설정 이전에 발급된 토큰(또는 탈퇴한 사용자)이면
//   무효 토큰과 똑같이 401 — JwtAuthGuard 와 동일한 기준을 적용한다.
export async function resolveOptionalUserId(
  ...
  try {
    payload = await jwtService.verifyAsync<JwtPayload>(value);
  } catch {
    throw unauthenticated();
  }

  if (!(await isTokenIssuedAfterPasswordChange(prisma, payload))) {
    throw unauthenticated();
  }
  return payload.sub;
}

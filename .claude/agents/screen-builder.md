...
**CSS 클래스** — `apps/web/app/globals.css`에 이미 정의됨. 레이아웃·상태 표현에 사용
...
- 버튼: `@/components/ui/button` 또는 `@/components/primitives`의 Button
- 아이콘: `@/components/icons`
- 공용 컴포넌트: `@/components/primitives` (TopBar, Logo, StatusPill)
- 타입: `@/types/` 또는 `packages/types`에서 임포트
...
구현 후 반드시 `pnpm --filter web tsc`로 타입 에러 확인.
